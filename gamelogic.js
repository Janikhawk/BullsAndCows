...
function checkedNumbers() {
	...
	*/
		...
		*/
		...
		var replay = confirm ('Победа!!!! Еще партию?');
			if (replay) {
				location.reload();
			} else {
				window.close();
			...
			tourGame = tourGame + 1;
			if (tourGame > 6) {
				var replay = confirm ('Вы проиграли... Правильный ответ: ' + A + B + C + D + '  Еще партию?');
				if (replay) {
					location.reload();
				} else {
					window.close();
		...
		//alert('Быков ' + Bulls); // !DEBUG отобразить количество Быков
		//alert('Коров ' + Cows);  // !DEBUG отобразить количество Коров
